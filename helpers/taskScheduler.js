...
module.exports = () => {
  const task = cron.schedule("0 0 1 * * *", async () => {
    ...
    const {
      count: count_admin,
      rows: rows_admin,
    } = await models.User.findAndCountAll({ where: { role: "admin" } });
    const name = "Product Activity Monthly Report";
    const {
      count: count_in,
      rows: rows_in,
    } = await models.Product_In.findAndCountAll({
    ...
    const {
      count: count_out,
      rows: rows_out,
    } = await models.Product_Out.findAndCountAll({
      where: sequelize.where(
        sequelize.fn("month", sequelize.col("date")),
        month
      ),
        ...
        attachments: [
          {
            ...
            path: `././asset/pdf/${monthNames[
              month - 1
            ].toLowerCase()}-${name.toLowerCase()}.pdf`,
