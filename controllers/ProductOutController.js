...
class Controller {
  static async create(req, res) {
    try {
      ...
      if (req.body.data.total > products.stock) {
        return res
          ...
          .json(
            response(
              "Fail",
              "Can't afford to check-out due to stock insufficient"
            )
          );
      }
      ...
      await models.Product.update(
        { stock: products.stock - req.body.data.total },
      ...
      return res
        .status(200)
        .json(response("Success", "Check out receipt is created", out));
    } catch (error) {
  ...
  static async find(req, res) {
    try {
      ...
      if (!find) {
        return res
          .status(422)
          .json(response("Fail", "Check-out data doesn't exist", find));
      }

      res
        .status(200)
        .json(response("Success", "Check-out data has been retrieved", find));
    } catch (error) {
      res.status(500).json(response("Fail", error.message));
    }
  }
  ...
  static async update(req, res) {
    try {
      ...
      res
        .status(200)
        .json(response("Success", "Checkout data is updated", data));
    } catch (error) {
      res.status(500).json(response("Fail", error.message));
    }
  }
